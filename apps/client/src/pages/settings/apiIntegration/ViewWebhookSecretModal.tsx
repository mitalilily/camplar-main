...
export const ViewWebhookSecretModal = ({
  ...
  const handleDownloadCredentials = () => {
    ...
    const fileName = `camplar-webhook-secret-${(webhook.name || 'webhook').replace(
      /\s/g,
      '-',
    )}-${timestamp.split('T')[0]}.txt`
    const fileContent = `
Camplar Webhook Secret
Generated: ${timestamp}
Webhook Name: ${webhook.name || 'Unnamed'}
Webhook URL: ${webhook.url}

WARNING: Keep this secret secure and never share it publicly.
      ...
      <Stack spacing={3}>
        ...
        >
          <Typography sx={{ fontWeight: 600, color: '#E65100', mb: 1, fontSize: '0.875rem' }}>
            Warning: Save this secret securely
          </Typography>
