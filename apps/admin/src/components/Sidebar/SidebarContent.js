...
import { CAMPLAR_BRAND, CAMPLAR_FONTS } from '../../utils/brand'
...
const SidebarContent = ({ logoText, routes, sidebarWidth }) => {
  ...
  const sidebarBg = useColorModeValue('rgba(0,11,55,0.96)', 'rgba(0,11,55,0.96)')
  const sidebarBorder = useColorModeValue('rgba(255,255,255,0.08)', 'rgba(255,255,255,0.16)')
  const sidebarShadow = useColorModeValue('14px 0 36px rgba(0,11,55,0.24)', '14px 0 36px rgba(0,11,55,0.38)')
  const activeBg = useColorModeValue('rgba(255,255,255,0.08)', 'rgba(255,255,255,0.12)')
  const hoverBg = useColorModeValue('rgba(255,255,255,0.06)', 'rgba(255, 255, 255, 0.08)')
  const activeBorder = useColorModeValue('rgba(255,255,255,0.16)', 'rgba(255,255,255,0.2)')
  const hoverBorder = useColorModeValue('rgba(255,255,255,0.12)', 'rgba(255,255,255,0.14)')
  const textColor = useColorModeValue('rgba(255,255,255,0.82)', 'gray.100')
  const iconColor = useColorModeValue('rgba(255,255,255,0.7)', 'gray.300')
  const dividerColor = useColorModeValue('rgba(255,255,255,0.12)', 'rgba(255,255,255,0.12)')
  const thumbColor = useColorModeValue('rgba(23,19,16,0.22)', 'rgba(255,255,255,0.24)')
  const brandText = useColorModeValue('white', 'gray.100')
  const mutedText = useColorModeValue('rgba(255,255,255,0.62)', 'rgba(255,255,255,0.66)')
  const collapsedLogoBg = useColorModeValue('rgba(255,255,255,0.08)', 'rgba(44,143,255,0.18)')
  const panelBg = useColorModeValue('rgba(255,255,255,0.04)', 'rgba(255,255,255,0.06)')
  const laneColor = useColorModeValue('rgba(255,255,255,0.12)', 'rgba(255,255,255,0.12)')
  ...
  return (
    <Box
      ...
      overflowX="hidden"
      pr="2"
      backgroundImage="radial-gradient(circle at 18% 4%, rgba(255,94,20,0.16) 0%, transparent 26%), radial-gradient(circle at 88% 9%, rgba(183,196,255,0.16) 0%, transparent 24%)"
      css={{
        scrollbarWidth: 'thin',
      ...
      <Box mb="18px" px="14px" textAlign="center" transition="all 0.3s ease">
        {showText ? (
          <Box
            ...
            py="14px"
            borderRadius="26px"
            bg="linear-gradient(180deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.04) 100%)"
            color="white"
            boxShadow="0 18px 36px rgba(0,11,55,0.18)"
            textAlign="left"
          >
            ...
            <Flex align="center" gap="12px" mb="12px">
              <Box bg="white" borderRadius="14px" p="10px">
                <Box as="img" src="/logo/camplar-mark.svg" alt="Camplar" h="26px" w="26px" objectFit="contain" />
              </Box>
              <Box>
                <Text fontFamily={CAMPLAR_FONTS.heading} fontWeight="800" fontSize="22px" letterSpacing="-0.04em">
                  {CAMPLAR_BRAND.uppercaseName}
                </Text>
                <Text fontSize="10px" color="rgba(255,255,255,0.62)" textTransform="uppercase" letterSpacing="0.18em">
                  Courier Aggregation
                </Text>
              </Box>
            </Flex>
